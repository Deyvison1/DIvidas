...
namespace Dividas.WebApi.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> Registrar(UserDto userDto)
        {
            ...
            {
                return this.StatusCode(StatusCodes.Status500InternalServerError, $"Erro no Registrar User. CODE{ex.Message}");
            }
        }
        [HttpPost("Login")]
        [AllowAnonymous]
        public async Task<IActionResult> Login(UserLoginDto userLogin)
        {
            try
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return Ok(new
                    {
                        token = GeradorJwtToken(appUser).Result,
                        user = userToReturn
                    });
                }
        ...
        private async Task<string> GeradorJwtToken(User user)
        {
            ...
            var key = new SymmetricSecurityKey(
                    Encoding.ASCII.GetBytes(_config
                        .GetSection("AppConfig:Token").Value));
